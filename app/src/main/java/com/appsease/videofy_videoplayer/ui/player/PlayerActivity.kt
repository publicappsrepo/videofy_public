...
import com.appsease.videofy_videoplayer.databinding.PlayerLayoutBinding
...
import com.appsease.videofy_videoplayer.ui.player.controls.PlayerControls
...
import com.appsease.videofy_videoplayer.utils.media.HttpUtils
import com.appsease.videofy_videoplayer.utils.media.SubtitleOps
...
class PlayerActivity :
   ...
   */
  private val playlistRepository: com.appsease.videofy_videoplayer.database.repository.PlaylistRepository by inject()
   ...
   */
  private lateinit var pipHelper: VideofyPipHelper
  ...
  private fun setupPipHelper() {
    pipHelper = VideofyPipHelper(activity = this, mpvView = player)
  }
  ...
  private fun copyMPVConfigFiles() {
    ...
    runCatching {
      ...
      File("$applicationPath/mpv.conf").apply {
        ...
        val content = advancedPreferences.videofyConf.get()
  ...
  private fun copyMPVScripts() {
    runCatching {
      ...
      if (advancedPreferences.enableLuaScripts.get()) {
        ...
        val mpvConfStorageUri = advancedPreferences.videofyConfStorageUri.get()
  ...
  internal fun onObserverEvent(
    ...
    when (property.substringBeforeLast("/")) {
      "user-data/videofy" -> viewModel.handleLuaInvocation(property, value)
    }
  }
    ...
    lifecycleScope.launch(Dispatchers.IO) {
      runCatching {
        ...
        val siblingFiles = if (launchSource == "video_list") {
          ...
          val videosInFolder = com.appsease.videofy_videoplayer.repository.MediaFileRepository.getVideosForBuckets(context, setOf(bucketId))
          val sortedVideos = com.appsease.videofy_videoplayer.utils.sort.SortUtils.sortVideos(videosInFolder, videoSortType, videoSortOrder)
  ...
  companion object {
     ...
     */
    private const val RESULT_INTENT = "com.appsease.videofy_videoplayer.ui.player.PlayerActivity.result"
