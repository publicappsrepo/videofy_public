package com.appsease.videofy_videoplayer.utils.history
...
object RecentlyPlayedOps {
  ...
  suspend fun getLastPlayedEntity(): com.appsease.videofy_videoplayer.database.entities.RecentlyPlayedEntity? {
    return withContext(Dispatchers.IO) {
      val recent = kotlin.runCatching { repository.getRecentlyPlayed(limit = 50) }.getOrDefault(emptyList())
      for (entity in recent) {
        val path = entity.filePath
